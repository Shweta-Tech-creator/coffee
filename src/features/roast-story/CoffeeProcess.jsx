...
const Step = ({ step, index }) => {
  ...
  return (
    <div ref={ref} className={`grid lg:grid-cols-2 gap-0 ${!isEven ? 'lg:grid-flow-dense' : ''}`}>
      {/* Image */}
      <motion.div
        className={`relative flex items-center justify-center p-8 lg:p-16 bg-[#08060A] overflow-hidden ${!isEven ? 'lg:col-start-2' : ''}`}
      ...
      >
        <motion.img
          src={step.img}
          alt={step.title}
          className="w-full max-w-[450px] aspect-[4/3] object-cover border border-[#C9A96E]/20"
          style={{ y: imgY }}
        />
      ...
      {/* Content */}
      <motion.div
        className={`flex flex-col justify-center bg-[#0A0810] px-10 lg:px-16 py-16 ${!isEven ? 'lg:col-start-1 lg:row-start-1' : ''}`}
        initial={{ opacity: 0, y: 40 }}
        animate={isInView ? { opacity: 1, y: 0 } : {}}
        transition={{ duration: 0.9, delay: 0.2 }}
      >
        <div className="flex items-center gap-4 mb-6">
          <span className="font-mono-custom text-[#C9A96E]/40 text-[10px] tracking-[0.4em]">{step.num}</span>
          <div className="h-px flex-1 bg-[#C9A96E]/10" />
          <span className="font-mono-custom text-[#5A5040] text-[10px] tracking-[0.3em] uppercase">{step.sub}</span>
        </div>

        <h3 className="font-display text-4xl lg:text-5xl text-[#EDE4D6] font-light mb-6 leading-tight">
          {step.title}
        </h3>

        <p className="text-[#6A6055] font-light leading-relaxed text-base mb-8">{step.body}</p>

        <div className="pl-4 border-l border-[#C9A96E]/30">
          <p className="font-mono-custom text-[10px] text-[#5A5040] tracking-wide">{step.detail}</p>
        </div>
      </motion.div>
    </div>
  );
...
export const CoffeeProcess = () => {
  ...
  return (
    <section id="process" ref={ref} className="bg-[#08060A]">
      {/* Header */}
        ...
        >
          <div>
            <div className="flex items-center gap-4 mb-6">
              ...
              <span className="font-mono-custom text-[#C9A96E] text-[10px] tracking-[0.4em] uppercase">The Craft</span>
            </div>
            <h2 className="font-display text-5xl lg:text-7xl text-[#EDE4D6] font-light leading-[1.05]">
              From Bean<br /><em className="text-[#C9A96E]">to Cup.</em>
            </h2>
          </div>
          <p className="text-[#5A5040] font-light max-w-sm leading-relaxed">
      ...
      <div className="border-t border-[#C9A96E]/8">
        {STEPS.map((step, i) => (
          <div key={i} className="border-b border-[#C9A96E]/8">
